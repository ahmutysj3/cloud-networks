resource "google_compute_instance" "jumpbox3" {
  description  = "test-jumpbox3"
  hostname     = null
  labels       = {}
  machine_type = "e2-micro"
  metadata = {
    ssh-keys = "trace:ssh-rsa  trace@trace-laptop"
  }


  name    = "instance-3"
  project = "terraform-project-trace-lab"
  tags    = []
  zone    = "us-east1-b"
  boot_disk {
    auto_delete = true
    device_name = "ubuntu-boot-disk3"
    mode        = "READ_WRITE"
    source      = google_compute_disk.ubuntu_boot3.self_link
  }

  attached_disk {
    device_name = "ubuntu-data-disk3"
    mode        = "READ_WRITE"
    source      = google_compute_disk.ubuntu_data3.self_link
  }

  network_interface {
    network    = google_compute_network.protected.self_link
    network_ip = cidrhost(google_compute_subnetwork.protected.ip_cidr_range, 4)
    nic_type   = "GVNIC"
    stack_type = "IPV4_ONLY"
    subnetwork = google_compute_subnetwork.protected.self_link
  }

  scheduling {
    automatic_restart           = false
    instance_termination_action = "DELETE"
    on_host_maintenance         = "TERMINATE"
    preemptible                 = true
    provisioning_model          = "SPOT"
  }

  service_account {
    email  = data.google_compute_default_service_account.default.email
    scopes = ["cloud-platform"]
  }

  shielded_instance_config {
    enable_integrity_monitoring = true
    enable_secure_boot          = false
    enable_vtpm                 = true
  }

}


resource "google_compute_disk" "ubuntu_boot3" {
  name                      = "ubuntu-boot-disk3"
  size                      = 10
  type                      = "pd-ssd"
  zone                      = data.google_compute_zones.available.names[0]
  image                     = data.google_compute_image.ubuntu.self_link
  physical_block_size_bytes = 4096
}

resource "google_compute_disk" "ubuntu_data3" {
  name                      = "ubuntu-data-disk3"
  size                      = 10
  type                      = "pd-ssd"
  zone                      = data.google_compute_zones.available.names[0]
  physical_block_size_bytes = 4096
}